...
import 'text!../../test/fixtures/providers.json';
import 'text!../../test/fixtures/provider_1.json';
...
testEnvModule.run( ($httpBackend) => {
    ...
    $httpBackend.whenGET(/http:\/\/apsed2427:8080\/api\/providers\?radius*/ )
        .respond( (method, url) => {
            console.log('url',url);
            if(url.contains('offset=0')) {
    ...
    $httpBackend.whenGET(/http:\/\/apsed2427:8080\/api\/providers\/specialties\?prefix*/ )
        .respond( (method, url) => {
            console.log('url',url);
